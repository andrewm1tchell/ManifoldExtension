...
contract test is AdminControl, CreatorExtension, ICreatorExtensionTokenURI, IManifoldERC721Edition {
    ...
    mapping(address => mapping(uint256 => mapping(uint256 => MyStruct))) _tokens;
    ...
    function generateTokenHash(address creator, uint256 series, uint256 tokenId) internal {
        ...
        uint128 rand = uint128(uint256(keccak256(abi.encodePacked(tokenId, block.timestamp, msg.sender))));
        _tokens[creator][series][tokenId].hash = tokenHash;
        _tokens[creator][series][tokenId].rand = rand;
    }
    ...
    function getTokenHash(address creator, uint256 series, uint256 tokenId) public view returns (bytes32){
        ...
        return _tokens[creator][series][tokenId].hash;
    }

    /**
     * @dev Rand value to generate art piece
     */
    function getRand(address creator, uint256 series, uint256 tokenId) public view returns (uint128){
        require(_totalSupply[creator][series] >= tokenId, "Token nonexistent");
        return _tokens[creator][series][tokenId].rand;
    }

     /**
     * @dev Token Hash used to generate art piece
     */
    function getTokenRand(address creator, uint256 series, uint256 tokenId) public view returns (uint128){
        require(_totalSupply[creator][series] >= tokenId, "Token nonexistent");
        return _tokens[creator][series][tokenId].rand;
    }
    ...
    function randConvert(uint256 rand) public pure returns (uint256[64] memory) {
        uint256[64] memory rands;
        for (uint256 i = 0; i < 64; ++i) {
            // Using keccak256 to generate a hash based on the original random number and the loop variable
            // Converting the hash to uint256
            // Using modulo 100 to get a number between 0 and 99
            rands[i] = uint256(keccak256(abi.encodePacked(rand + i))) % 100;
        }
